		...
		public TableLayoutConstraints(String constraints)
			{
				...
				// Use default values for any parameter not specified or
				// specified
				...
				try
					{
						// Get the first column (assume component is in only one
						// column)
						token = st.nextToken();
						col1 = new Integer(token).intValue();
						col2 = col1;

						// Get the first row (assume component is in only one
						// row)
						token = st.nextToken();
				...
				catch (NumberFormatException error)
					{
						try
							{
								// Check if token means horizontally
								// justification the component
								...
								// There can be one more token for the vertical
								// justification even
