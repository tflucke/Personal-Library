package name.tomflucke.layouts;
 ...
 *     double border = 10;
 *     double size[][] =
 *         {{border, 0.10, 20, Tablelayouts.FILL, 20, 0.20, border},  // Columns
 *          {border, 0.20, 20, Tablelayouts.FILL, 20, 0.20, border}}; // Rows
 * <spc>
...
public class TableLayout implements java.awt.LayoutManager2, java.io.Serializable,
		name.tomflucke.layouts.TableLayoutConstants, NoZOrder
{

	private static final long serialVersionUID = -5912155526970148295L;
	 ...
	 * columnOffset[n] and the right boarder is at columnOffset[n + 1] for
	 * all columns including the last one. columnOffset.length =
	 * columnSize.length + 1
	 */
	 ...
	 * known size of the container. If dirty is true or the container has
	 * been resized, the cell sizes must be recalculated using
	 * calculateSize.
	 */
	...
	/**
	 * Constructs an instance of Tablelayouts. This TableLayout will have
	...
	/**
	 * Constructs an instance of Tablelayouts.
	 * 
	 * @param size
	 *            widths of columns and heights of rows in the format,
	 *            {{col0, col1, col2, ..., colN}, {row0, row1, row2, ...,
	 *            rowM}} If this parameter is invalid, the TableLayout will
		...
		if ((size != null) && (size.length == 2))
		{
			...
			for (int counter = 0; counter < rowSpec.length; counter++)
				if ((rowSpec[counter] < 0.0) && (rowSpec[counter] != FILL)
						&& (rowSpec[counter] != PREFERRED) && (rowSpec[counter] != MINIMUM))
				{
					rowSpec[counter] = 0.0;
				}
		}
	 ...
	 *            desired component
	 * 
	 * @return If the given component is found, the constraints associated
	 *         with that component. If the given component is null or is not
	...
	public TableLayoutConstraints getConstraints(Component component)
	{
		...
		while (iterator.hasNext())
		{
			...
			if (entry.component == component)
				return new TableLayoutConstraints(entry.col1, entry.row1, entry.col2, entry.row2,
	...
	/**
	 * Adjusts the number and sizes of rows in this layouts. After calling
	 * this method, the caller should request this layout manager to perform
	 * the layouts. This can be done with the following code:
	 * 
	 * <pre>
	 * layouts.layoutContainer(container);
	 * container.repaint();
	 * </pre>
	 * 
	 * or
	 ...
	 * </pre>
	 * 
	 * If this is not done, the changes in the layout will not be seen until
	...
	/**
	 * Adjusts the width of a single column in this layouts. After calling
	 * this method, the caller should request this layout manager to perform
	 * the layouts. This can be done with the following code:
	 * 
	 * <code>
	 ...
	 * 
	 * @param i
	 *            zero-based index of column to set. If this parameter is
	...
	/**
	 * Adjusts the height of a single row in this layouts. After calling
	 * this method, the caller should request this layout manager to perform
	 * the layouts. This can be done with the following code:
	 * 
	 * <code>
	...
	/**
	 * Gets the sizes of columns in this layouts.
	 * 
	...
	/**
	 * Gets the height of a single row in this layouts.
	 * 
	...
	/**
	 * Gets the width of a single column in this layouts.
	 * 
	 * @param i
	...
	/**
	 * Gets the sizes of a row in this layouts.
	 * 
	 * @param i
	 *            zero-based index of row to get. If this parameter is not
	 *            valid, an ArrayOutOfBoundsException will be thrown.
	...
	/**
	 * Gets the number of columns in this layouts.
	 * 
	...
	/**
	 * Gets the number of rows in this layouts.
	 * 
	...
	/**
	 * Inserts a column in this layouts. All components to the right of the
	 * insertion point are moved right one column. The container will need
	 * to be laid out after this method returns. See <code>setColumn</code>.
	 * 
	 * @param i
	...
	/**
	 * Inserts a row in this layouts. All components below the insertion
	 * point are moved down one row. The container will need to be laid out
	 * after this method returns. See <code>setRow</code>.
	 * 
	 * @param i
	 *            zero-based index at which to insert the column.
	 * @param size
	...
	/**
	 * Deletes a column in this layouts. All components to the right of the
	...
	/**
	 * Deletes a row in this layouts. All components below the deletion
	 * point are moved up one row. The container will need to be laid out
	 * after this method returns. See <code>setRow</code>. There must be at
	 * least two rows in order to delete a row.
	 * 
	 * @param i
	 *            zero-based index of column to delete
	 * 
	...
	/**
	 * Draws a grid on the given container. This is useful for seeing where
	 * the rows and columns go. In the container's paintComponent method,
	 * call this method.
	 * 
	 * @param container
	 ...
	 *            graphics content of container (can be offscreen)
	 * @param client
	 *            Whether or not to draw for client mode or developer mode
	 */
	public void drawGrid(Container container, Graphics g, boolean client, boolean eadges)
	{
		if (client)
		{
			// Calculate the sizes of the rows and columns
			...
			for (int row = 0; row < rowSize.length; row++)
			{
				...
				for (int column = 0; column < columnSize.length; column++)
				{
					// Draw the cell as a solid rectangle
					if (eadges
							|| (column != 0 && row != 0 && row != rowSize.length - 1 && column != columnSize.length - 1))
					{
						g.drawRect(x, y, columnSize[column], rowSize[row]);
					}
					else
					{
						if ((row == 0 || row == rowSize.length - 1)
								&& column < columnSize.length - 1)
						{
							g.drawLine(x + columnSize[column], y, x + columnSize[column], y
									+ rowSize[row]);
						}
						if ((column == 0 || column == columnSize.length - 1)
								&& row < rowSize.length - 1)
						{
							g.drawLine(x, y + rowSize[row], x + columnSize[column], y
									+ rowSize[row]);
						}
					}
	 ...
	 * configuration. Such a component is, at least partly, in an invalid
	 * row or column. For example, on a table with five rows, row -1 and row
	 * 5 are both invalid. Valid rows are 0 through 4, inclusively.
	 * 
	 ...
	 * 
	 * @return True, if there are any overlapping components. False,
	 *         otherwise.
	 * 
	 * @see hidden
	 */
	...
	/**
	 * Calculates the sizes of the rows and columns based on the absolute
	 * and relative sizes specified in <code>rowSpec</code> and
	...
	protected void calculateSize(Container container)
	{
		...
		// Assign preferred and minimum widths; this reduces available
		// width.
		// Assignment of preferred/minimum with is like assignment of
		// absolute
		...
		for (counter = 0; counter < numColumn; counter++)
			...
			if ((columnSpec[counter] == PREFERRED) || (columnSpec[counter] == MINIMUM))
			{
				...
				// Find maximum preferred width of all components
				// completely
				...
				while (iterator.hasNext())
				{
					...
					if ((entry.col1 == counter) && (entry.col2 == counter))
					{
						Dimension p = (columnSpec[counter] == PREFERRED) ? entry.component
								.getPreferredSize() : entry.component.getMinimumSize();
		...
		// Assign preferred and minimum heights; this reduces available
		// height.
		// Assignment of preferred/minimum with is like assignment of
		// absolute
		// heights except that each row must determine the maximum
		// preferred/minimum height of the components that are
		...
		for (counter = 0; counter < numRow; counter++)
			...
			if ((rowSpec[counter] == PREFERRED) || (rowSpec[counter] == MINIMUM))
			{
				...
				// Find maximum preferred height of all components
				// completely
		...
		// Remember how much space is available for relatively sized
		// cells
		...
		// statements
		// will always evaluate to false and the division will not
		// occur.

		// If there are more than one "fill" cell, slack may occur due
		// to rounding
		// errors
		...
		// Indicate that the size of the cells are known for the
		// container's
		// current size
		dirty = false;
		oldWidth = totalWidth;
	...
	/**
	 * To lay out the specified container using this layouts. This method
	...
	public void layoutContainer(Container container)
	{
		...
		// Calculate sizes if container has changed size or components
		// were added
		Dimension d = container.getSize();
		...
		for (int counter = 0; counter < component.length; counter++)
		{
			try
			{
				...
				// Skip any components that have not been place
				// in a specific cell
				if (entry == null)
					break;
				...
				if (entry.singleCell)
				{
					// The following block of code has been
					// optimized so that the
					// preferred size of the component is
					// only obtained if it is
					// needed. There are components in which
					// the getPreferredSize
					// method is extremely expensive, such
					// as data driven controls
					// with a large amount of data.

					// Get the preferred size of the
					// component
					...
					{
						case LEFT:
							// Align left side along left
							// edge of cell
							x = columnOffset[entry.col1];
							break;

						case RIGHT:
							// Align right side along right
							// edge of cell
					...
					if ((entry.vAlign == FULL) || (cellHeight < preferredHeight))
						...
						// Use the prefered height of the
						// component
						h = preferredHeight;
				...
				{
					// Align left side with left boarder of
					// first column
					x = columnOffset[entry.col1];

					// Align top side along top edge of
					// first row
					y = rowOffset[entry.row1];

					// Align right side with right boarder
					// of second column
					w = columnOffset[entry.col2 + 1] - columnOffset[entry.col1];

					// Align bottom side with bottom boarder
					// of second row
	 ...
	 * layouts. The preferred size is the smallest size that, if used for
	 * the container's size, will ensure that all components are at least as
	 * large as their preferred size. This method cannot guarantee that all
	 * components will be their preferred size. For example, if component A
	 * and component B are each allocate half of the container's width and
	 * component A wants to be 10 pixels wide while component B wants to be
	 * 100 pixels wide, they cannot both be accommodated. Since in general
	 * components rather be larger than their preferred size instead of
	 * smaller, component B's request will be fulfilled. The preferred size
	 * of the container would be 200 pixels.
	 * 
	 * @param container
	 *            container being served by this layout manager
	 * 
	...
	public Dimension preferredLayoutSize(Container container)
	{
		...
		// Determine percentage of space allocated to fill components.
		// This is
		...
		for (counter = 0; counter < columnSpec.length; counter++)
			...
			if ((columnSpec[counter] == PREFERRED) || (columnSpec[counter] == MINIMUM))
			{
				...
				// Find maximum preferred/minimum width of all
		...
		while (iterator.hasNext())
		{
			...
			if ((entry.col1 < 0) || (entry.col1 >= columnSpec.length)
					...
					|| (entry.row1 >= rowSpec.length) || (entry.row2 >= rowSpec.length))
			{
			...
			// Calculate portion of component that is not absolutely
			// sized
			...
			// Determine total percentage of scalable space that the
			// component
			// occupies by adding the relative columns and the fill
			// columns
			...
			// Determine the total scaled width as estimated by this
			// component
			if (relativeWidth == 0)
				temp = 0;
			else
		...
		// totalWidth is the scaledWidth plus the sum of all absolute
		// widths and all
		// preferred widths
		...
		// totalHeight is the scaledHeight plus the sum of all absolute
		// heights and
	 ...
	 * 
	 * @param i
	 *            The index of the row of which to get the size
	 * @return The size of the row in pizels
	 */
	public int getRowSize(int i)
	{
		if (i > rowSize.length || i < 0)
		{
			return -1;
		}
		else
	 ...
	 * 
	 * @param i
	 *            The index of the column of which to get the size
	 * @return The size of the column in pizels
	 */
	public int getColumnSize(int i)
	{
		if (i > columnSize.length || i < 0)
		{
			return -1;
		}
		else
	 ...
	 * layouts. The minimum size is the smallest size that, if used for the
	 * container's size, will ensure that all components are at least as
	 * large as their minimum size. This method cannot guarantee that all
	 * components will be their minimum size. For example, if component A
	 * and component B are each allocate half of the container's width and
	 * component A wants to be 10 pixels wide while component B wants to be
	 * 100 pixels wide, they cannot both be accommodated. Since in general
	 * components rather be larger than their minimum size instead of
	 * smaller, component B's request will be fulfilled. The minimum size of
	...
	/**
	 * Adds the specified component with the specified name to the layouts.
	 * 
	 * @param name
	...
	public void addLayoutComponent(Component component, Object constraint)
	{
		...
		{
			// Create an entry to associate component with its
			// constraints
	...
	/**
	 * Removes the specified component from the layouts.
	 * 
	 * @param component
	...
	@Override
	public Object getConstrains(Component comp)
	{
		TableLayoutConstraints constr = getConstraints(comp);
		return constr.col1 + "," + constr.row1 + "," + constr.col2 + "," + constr.row2;
	}
	...
	protected class Entry extends TableLayoutConstraints
	{
		...
		/**
		 * Constructs an Entry that binds a component to a set of
		 * constraints.
		 * 
		 * @param component
		...
		{
			super(constraint.col1, constraint.row1, constraint.col2, constraint.row2,
		 ...
		 * 
		 * @param object
		 *            object being compared to; must be a Component if
		 *            it is equal to this TableLayoutConstraints.
		 * 
		 * @return True, if the entries refer to the same component
